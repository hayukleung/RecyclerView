...
/**
 * RecyclerView
 * com.hayukleung.recyclerview.LayoutManager
 * DemoLayoutManager.java
 *
 * by hayukleung
 * at 2017-05-21 10:37
 */
...
public class DemoLayoutManager extends RecyclerView.LayoutManager {
  ...
  @Override public int scrollHorizontallyBy(int dx, RecyclerView.Recycler recycler,
    ...
    if (horizontalOffset + dx < 0) {
      // 滑动到最左边
      dx = -horizontalOffset;
    } else if (horizontalOffset + dx > mTotalWidth - getHorizontalSpace()) {
      // 滑动到最右边
  ...
  private void fill(RecyclerView.Recycler recycler, RecyclerView.State state) {
    ...
    // 在屏幕上显示出
    for (int i = 0; i < getItemCount(); i++) {
      if (Rect.intersects(displayFrame, itemFrames.get(i))) {
        // 判断是否在屏幕中
        View view = recycler.getViewForPosition(i);
        measureChildWithMargins(view, 0, 0);
        addView(view);
